...
class _ConversationScreenState extends State<ConversationScreen> {
  ...
  void _handleStreamData(Map<String, dynamic> data) {
    ...
    if (data.containsKey('sources')) {
      ...
      // Check event type to ensure we only update sources for search-results
      // This aligns with Web logic which doesn't mix agent-read-results into the main source list
      final eventType =
          data['event_type'] ?? data['agent_response']?['event_type'];

      if (sources != null && eventType == 'search-results') {
        setState(() {
          final currentMessage = _messages[messageIndex];
          // Use replace strategy instead of merge for search-results to ensure
          // we align with the source of truth from backend.
          final newSources = _dedupeSources(sources);
          _messages[messageIndex] = ConversationMessage(
            id: currentMessage.id,
    ...
    switch (eventType) {
            ...
            setState(() {
              ...
              final newStepResults = results.whereType<Map>().map((r) {
                ...
                return AgentStepResult(
                  title:
                      r['title']?.toString() ?? S.current.source_default_title,
                  url: url,
      ...
      case 'agent-call-tool':
        ...
        if (stepsDetails != null && stepsDetails.isNotEmpty) {
          final latestStep = stepsDetails.last as Map<String, dynamic>;
          final stepName =
              latestStep['step'] as String? ?? S.current.step_processing;
    ...
    while (_currentAgentSteps.length <= stepNumber) {
      _currentAgentSteps.add(AgentStep(
        ...
        title: _currentQueryPlan != null &&
            ...
            : '${S.current.step_prefix} ${_currentAgentSteps.length + 1}',
      ));
    }
  ...
  void _updateMessageAgentSteps(int messageIndex) {
    ...
    // Aggregate sources from all steps
    // Note: We no longer merge agent steps into the main sources list to align with Web logic.
    // The main sources list should only contain sources from 'search-results'.
    // Agent results are displayed within the agent steps UI only.
    ...
    _messages[messageIndex] = ConversationMessage(
      ...
      relatedQueries: currentMessage.relatedQueries,
      sourcesCount: currentMessage.sourcesCount,
      sources: currentMessage.sources, // Keep existing sources (search-results)
